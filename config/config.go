	...
	return &config{
		app: &app{
			...
			readTimeout: func() time.Duration {
				...
				return time.Duration(int64(rt) * int64(math.Pow10(9)))
			}(),
			writeTimeout: func() time.Duration {
				rt, err := strconv.Atoi(envMap["APP_WRTIE_TIMEOUT"])
				if err != nil {
			...
			bodyLimit: func() int {
				bl, err := strconv.Atoi(envMap["APP_BODY_LIMIT"])
				if err != nil {
			...
			fileLimit: func() int {
				fl, err := strconv.Atoi(envMap["APP_FILE_LIMIT"])
				if err != nil {
		...
		jwt: &jwt{
			...
			accessExpiresAt: func() int {
				ae, err := strconv.Atoi(envMap["JWT_ACCESS_EXPIRES"])
				if err != nil {
			...
			refreshExpiresAt: func() int {
				re, err := strconv.Atoi(envMap["JWT_REFRESH_EXPIRES"])
				if err != nil {
